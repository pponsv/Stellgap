...
c     (in file tae_data_vmec) calculated by xmetric.
...
c      integer, parameter :: irads = 89, irad3 = 3*irads
     ...
     1   aion, bion, cion
      ...
      real(kind=rprec), parameter :: gamma = 1.7d0
...
c      write(*,*) outfile
      ...
      allocate(phi_norm(mn_col), stat=istat)
      allocate(psi_norm(mn_col), stat=istat)
      if(mype .eq. 0) then
