!
!  this code reads data from alfven_post and converts it
!   to a .silo file for use in Visit. Note below that independent
!   variable axis can be either tor_flux or sqrt(tor_flux). Data
!   coming in from stellgap, post_process is in tor_flux.
!
implicit none
include&
...
!       read(4,*) r(i),omega(i),idum,jdum,m(i),n(i)
   omega(i) = omega(i)*freq_norm
!        if(omega(i) .lt. 300.) then
   ic = ic + 1
   w(ic) = omega(i)
!         rr(ic) = r(i)        !plot vs. psi
   rr(ic) = sqrt(r(i))  !plot vs. sqrt(psi)
!         rr(ic) = -0.00087078 + 1.1526*sqrt(r(i)) - 0.11225*r(i)
!     >            + 0.21002*r(i)*sqrt(r(i)) -0.24732*r(i)*r(i)  !plot vs. sqrt(psi-pol)
   nn(ic) = n(i)
   mm(ic) = m(i)
...
!
!     open and write silo formatted file for Visit
!
